...
public class UvcCameraController {
    ...
    public void init(MainActivity activity) {
        ...
        if (null != mWeakReference) {
            ...
            if (null != mainActivity) {
                ...
                // 参数初始化赋值,实现onAttch,onDettach等函数
                ...
                Log.d("UvcCameraController", "init deviceFilters"+deviceFilters.get(0).toString());

                Log.d("UvcCameraController", "init cout: " + mUsbDeviceList.size());
            }
        }
    }
    ...
    public void checkPermissionOpenCamera() {
        // 申请权限并打开摄像头
        Log.d("checkPermissionOpenCamera", "checkPermissionOpenCamera count: " + mUsbDeviceList.size());
...
//            Toast.makeText(MainActivity.this, "USB_DEVICE_DETACHED", Toast.LENGTH_SHORT).show();
        ...
        public void onConnect(UsbDevice device, USBMonitor.UsbControlBlock ctrlBlock, boolean createNew) {
            Log.d("onConnect", "onConnect: 设备连接成功：" + device.getDeviceName());
            Log.d("onConnect", "onConnect: 设备数量：" + mUvcCameraHandlerList.size());
            //设备连接成功
            for (int i = 0; i < mUvcCameraHandlerList.size(); i++) {
                UVCCameraHandler uvcCameraHandler = mUvcCameraHandlerList.get(i);
                if (!uvcCameraHandler.isOpened()) {
