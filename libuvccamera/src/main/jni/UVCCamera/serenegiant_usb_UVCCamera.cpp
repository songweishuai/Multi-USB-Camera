...
static jint nativeSetPreviewSize(JNIEnv *env, jobject thiz,
	...
	ENTER();
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	LOGE("nativeSetPreviewSize width:%d",width);
	LOGE("nativeSetPreviewSize height:%d",height);
	LOGE("nativeSetPreviewSize min_fps:%d",min_fps);
	LOGE("nativeSetPreviewSize max_fps:%d",max_fps);
	LOGE("nativeSetPreviewSize mode:%d",mode);
	LOGE("nativeSetPreviewSize bandwidth:%d",bandwidth);


	if (LIKELY(camera)) {
		LOGE("nativeSetPreviewSize bandwidth  1");
